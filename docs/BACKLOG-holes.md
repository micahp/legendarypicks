...
| 164 | **130 prop_games never linked to ESPN, and nothing recorded a single failure.** `/api/props/ingest` wrapped the ESPN crosswalk in `except Exception: pass`, commented "best-effort; don't block ingest". Settlement keys on that link for every league except MLB, so every prop on an unlinked game is unsettleable. Sibling of 104: same join, measured across all leagues. | prod 2026-09-06: **130 unlinked games carrying 1,828 props, 1,828 of them unsettled (100%)**. atp 713, wta 505, lcup 249, ncaaf 186, mls 164. Two causes, not one: ncaaf/mls have a linked twin on the same date (exact-string join misses "Michigan (#16)" against "Michigan Wolverines"), while tennis reads linked=0 for whole days including US Open week, where linking never happened at all. | `_try_link_prop_game` now logs the exception type and message, or the candidate count on a miss, and every ingest response carries a `link` outcome (linked / no_match / failed / already_linked). Still best-effort by design, because a serving path must not wait on a publisher. **The 1,828 existing props are NOT repaired by this**: that needs `link_prop_games.py` scoped per league, then `dedupe_prop_games.py` to fold the twins, and the deduper deliberately refuses rows with no event id. |

| 165 | ~~`ingest_nfl_projections.py` cannot run unattended~~ **WITHDRAWN 2026-09-06, same day it was raised.** It was filed as an orphaned ingest that lost its timer. It is not one. Its header says it reads a PINNED snapshot and "never a live projection fetch", with a contract (`REG-projection-source`) requiring every row to reference `payload_checksum`, so the pin is the design: a mock-draft pool must not move under the people using it. | `nfl_player_projections` newest 2026-08-03, judged "34 days stale". That applied an in-season yardstick to a SEASON-long projection (`scoringPeriodId=0`, `statSourceId=1`). A season projection not moving is the feature. | Nothing to fix. The one real question left is narrower: the pinned snapshot is gitignored and ABSENT on prod, so the refresh path cannot be exercised there at all. Decide whether the snapshot should be a tracked artifact, not whether the job should be scheduled. |
