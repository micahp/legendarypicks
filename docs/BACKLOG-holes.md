...
| 165 | **`ingest_nfl_projections.py` cannot run unattended: it requires a hand-dropped, gitignored snapshot.** It is the only orphaned ingest that could not be adopted into `ingest_registry` on 2026-09-06, because a scheduled job that can only fail is worse than no job. | `_load_snapshot()` raises `pinned snapshot missing: backend/data/espn_2026_snapshot_page1.json`; the path is gitignored at `.gitignore:80` and absent on prod. `nfl_player_projections` newest row 2026-08-03, 34 days stale, against a season that started 2026-08-29. | Either make it fetch page 1 itself (the ESPN fantasy endpoint it already reads for `proTeamSchedules_wl` is in the same file), or document the drop as a deliberate manual step and give it a freshness target so the staleness is visible. Do not schedule it as-is. |
