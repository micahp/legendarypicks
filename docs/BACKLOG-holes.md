...
| # | defect | evidence | fix |
|---|---|---|---|
| 104 | **913 tennis props cannot reach a game page.** 274 of 325 prod ATP/WTA `prop_games` carry no `espn_event_id`, and the game page joins on it. **The 2026-08-18 figure of 2,475 props was not reproducible on 08-20** and is not explained by the 08-19 dedupe alone; re-measure before quoting a tennis number. | prod tennis `prop_games` **51 of 325 linked (16%)**; current dev (2026-08-21) **53 of 63 linked**, or **907 of 1,017 props**. The remaining 110 props cannot settle from the event source. | `link_prop_games.py` for atp/wta, then re-measure. Note `reference_espn_folds_tennis_names`: ESPN folds accents in tennis but not soccer, so a name join behaves differently per sport. |
| 105 | **Tennis settles nothing at all in managed data.** Every ATP and WTA prop on both databases still has no outcome. The board shows a line and never says how it landed. | prod atp **0 of 536**, wta **0 of 377**; current dev atp **0 of 556**, wta **0 of 461**. Candidate `f4f8cd2` grades the four published player markets only after an exact ESPN event link and completed score. | land and clone-rehearse the grader, then run only linked final games. Keep missing/retired source results retryable; do not stamp voids as settlements. |
