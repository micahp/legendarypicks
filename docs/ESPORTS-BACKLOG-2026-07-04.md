...
## 5. Tier system is global, should be per-game + 4-tier taxonomy upgrade — `docs/LEAGUE-REGISTRY-per-game.md`

Parallel work (different session/pane, commit `46c5d71`) found a real structural gap in the tier
system shipped this session: `league_tier.py`'s keyword tuples are GLOBAL, but a league's tier is
relative to its game — European Pro League is Semi-Pro/Minor for CS2 but Amateur/Development for
Dota (Liquipedia Tier-3 there), so the same keyword misfires across titles. Worse: every game's
actual top-tier majors except EWC/MSI/CDL are entirely unclassified (LCK, VCT International,
BLAST/ESL/IEM, TI, Six Invitational) — they'd default to the neutral tier the day they appear and
sort *below* CCT/Prime League, a real landmine.

Also upgraded the taxonomy itself (Micah's ask, researched against Liquipedia's real S/A/B/C/D
convention + confirmed showmatch is its own industry category, not a rung of "amateur"): the old
3-bucket Major/Feeder/Noise scheme conflated real regional-pro leagues (CCT, European Pro League)
with nascent development circuits (TESFED, United21, XSE) under one "Feeder" label. Now **4 tiers**:
Major/Pro (0) — Semi-Pro/Minor (1) — Amateur/Development (2) — Novelty/Exhibition (3), with every
current league on the board re-classified per-game against it.
