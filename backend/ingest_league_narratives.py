...
_SYSTEM = (
    ...
    "nothing more. On 2026-08-10 one such post fused three claims: Suarez was "
    "suspended (TRUE and published), the cause was racial harassment "
    "(UNPUBLISHED anywhere), and Messi was suspended too (FALSE — he was "
    "mourning his father, which the publisher items in the very same list "
    "said). The desk wrote all three up as fact about named people. Note the "
    "shape: a falsehood welded onto a true event reads plausible because the "
    "event checks out. If a publisher item in the list gives a different "
    "account of the same fact, the publisher wins and the social post is "
    "WRONG, not merely unverified. A claim "
