#!/usr/bin/env python3
"""Plan, but never apply, an additive UFCStats history merge from dev to production.

Both databases are opened with SQLite ``mode=ro``. The identity bridge is
``players.espn_id`` (numeric ESPN athlete ID), never ``source_player_key``
(which is a UFCStats hex hash in this table). Existing production values always win.

This is the ``player_game_logs_ufcstats`` sibling of ``plan_ufc_history_merge.py``.
"""
...
class SourceIdentity:
    athlete_id: str        # ESPN numeric ID from players.espn_id
    name: str
    dev_player_id: int
    source_player_key: str  # UFCStats hex hash
...
@dataclass(frozen=True)
class PlannedLog:
    athlete_id: str        # ESPN numeric ID for prod player lookup
    source_player_key: str  # UFCStats hex hash for the column value
    source_name: str
    dev_player_id: int
    season: int
    game_no: str
...
class MergePlan:
    ...
    skipped_no_espn: int = 0


def _natural_key(row: sqlite3.Row):
    """Unique key for a ufcstats log: (source_player_key, game_id)."""
    return (str(row["source_player_key"]), str(row["game_id"]))
...
def _rows_equivalent(source: sqlite3.Row, prod: sqlite3.Row) -> bool:
    ...
    if source_stats is None or prod_stats is None or source_stats != prod_stats:
        return False
    fields = (
        "game_id", "game_date", "opponent", "season", "game_no",
        "source", "source_player_key", "source_event_key",
    )
    return all(
...
def _load_source_rows(con: sqlite3.Connection) -> List[sqlite3.Row]:
    return con.execute(
        ...
        FROM player_game_logs_ufcstats l
        JOIN players p ON p.id=l.player_id
        WHERE l.league='ufc'
        ORDER BY l.source_player_key, l.game_id, l.id
        """
    ).fetchall()
...
def _load_prod_logs(con: sqlite3.Connection):
    return {
        ...
        for row in con.execute(
            """
            SELECT * FROM player_game_logs_ufcstats
            WHERE league='ufc' AND source_player_key IS NOT NULL AND game_id IS NOT NULL
            """
        )
    }
...
def _source_identities(
    ...
    """Key by dev_espn_id (numeric ESPN ID from players table)."""
    identities: Dict[str, SourceIdentity] = {}
    for row in rows:
        athlete_id = str(row["dev_espn_id"] or "").strip()
        source_player_key = str(row["source_player_key"] or "").strip()
        game_id = str(row["game_id"] or "").strip()
        stats = _json_value(row["stats"])
        if (
            not athlete_id
            or not source_player_key
            or not game_id
            or stats is None
            or row["source"] != "ufcstats"
        ):
            plan.skipped_no_espn += 1
            continue
...
def build_merge_plan(
    ...
    espn_to_spk = {v.athlete_id: v.source_player_key for v in identities.values()}
    for row in source_rows:
        athlete_id = str(row["dev_espn_id"] or "").strip()
        if not athlete_id or athlete_id not in identity_actions:
            continue
        ...
        if existing is not None:
            ...
            continue
        spk = espn_to_spk.get(athlete_id, str(row["source_player_key"] or "").strip())
        plan.planned_logs.append(
            PlannedLog(
                athlete_id=athlete_id,
                source_player_key=spk,
...
def print_plan(plan: MergePlan, prod_integrity: str, dev_integrity: str) -> None:
    print("UFCStats dev-history additive merge plan (READ-ONLY)")
    print("  prod integrity: {}".format(prod_integrity))
    print("  dev integrity: {}".format(dev_integrity))
    print("  dev UFCStats source rows: {}".format(plan.source_rows))
    print("  dev UFCStats fighter identities: {}".format(plan.source_fighters))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan a read-only additive UFCStats history merge from dev to prod"
    )
