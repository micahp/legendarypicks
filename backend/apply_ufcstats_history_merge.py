#!/usr/bin/env python3
"""Apply one previously counted UFCStats dev-history merge in a short transaction.

This companion to ``plan_ufcstats_history_merge.py`` requires:
...
from plan_ufcstats_history_merge import (
    DEFAULT_DEV,
    DEFAULT_PROD,
    IdentityAction,
    MergePlan,
...
def apply_merge_plan(prod_path: str, plan: MergePlan) -> dict:
    ...
    try:
        ...
        con.executemany(
            """
            INSERT OR IGNORE INTO player_game_logs_ufcstats
              (player_id, league, season, game_no, game_id, game_date, opponent,
               stats, source, source_player_key, source_event_key, ingested_at)
            VALUES(?, 'ufc', ?, ?, ?, ?, ?, ?, 'ufcstats', ?, ?, COALESCE(?, datetime('now')))
            """,
            [
                (
