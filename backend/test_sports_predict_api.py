...
def test_slate_includes_live_games_and_nearest_upcoming_day(tmp_path):
    path = tmp_path / "predict.db"
    _create_db(path)
    now = dt.datetime.now(dt.timezone.utc)
    live_start = now - dt.timedelta(minutes=30)
    next_start = now + dt.timedelta(hours=2)
    later_start = now + dt.timedelta(days=1)
    with closing(_connection(path)) as con:
        for game_id, start, state in (
            ("live", live_start, "in"),
            ("next", next_start, "pre"),
            ("later", later_start, "pre"),
        ):
            iso = start.isoformat()
            con.execute("INSERT INTO scoreboard_snapshots VALUES(?,?,?,?,?,?,?)", (
                "mlb", start.date().isoformat(), game_id,
                _payload(game_id, iso, state), state, iso, iso,
            ))
        con.commit()

    with patch.object(games, "_db", lambda: _connection(path)):
        slate, source = predictions._sports_slate("mlb", int(now.timestamp() * 1000))

    assert source == "scoreboard_snapshots"
    assert [match["gameId"] for match in slate] == ["live", "next"]
    assert [match["live"] for match in slate] == [True, False]


def test_grouped_slate_combines_leagues_and_grouped_ledger_filters(tmp_path):
    path = tmp_path / "predict.db"
    _create_db(path)
    now = dt.datetime.now(dt.timezone.utc)
    ...
    with closing(_connection(path)) as con:
        for league in ("mls", "lcup"):
            game_id = f"{league}-one"
            con.execute("INSERT INTO scoreboard_snapshots VALUES(?,?,?,?,?,?,?)", (
                league, start.date().isoformat(), game_id,
                _payload(game_id, iso), "pre", iso, iso,
            ))
            ...
            ) VALUES(?,?, 'x',?,'device',?,'A','Away','Home',?,1,2)""", (
                league, game_id, iso, f"{league}:{game_id}", start.date().isoformat(),
            ))
        con.execute("""INSERT INTO predictions(
            league,game_id,predicted_winner,created_at,device_id,match_key,side,
            team_a,team_b,event_date,created_at_ms,lock_at
        ) VALUES('nba','nba-one','x',?,'device','nba:nba-one','A','Away','Home',?,1,2)""", (
            iso, start.date().isoformat(),
        ))
        con.commit()

    with patch.object(games, "_db", lambda: _connection(path)):
        slate = predictions.sports_predict_slate(leagues="mls,lcup,wc")
        picks = _body(predictions.get_my_sports_picks(None, "device", "mls,lcup,wc"))

    assert {match["league"] for match in slate["matches"]} == {"mls", "lcup"}
    assert {pick["league"] for pick in picks["picks"]} == {"mls", "lcup"}


def test_wnba_is_not_an_offered_prediction_league():
    with pytest.raises(HTTPException) as raised:
        predictions.sports_predict_slate(league="wnba")
...
def test_live_game_is_visible_but_rejected_by_pick_endpoint(tmp_path):
    path = tmp_path / "predict.db"
    _create_db(path)
    start = dt.datetime.now(dt.timezone.utc) - dt.timedelta(minutes=15)
    iso = start.isoformat()
    with closing(_connection(path)) as con:
        con.execute("INSERT INTO scoreboard_snapshots VALUES(?,?,?,?,?,?,?)", (
            "nhl", start.date().isoformat(), "live-one",
            _payload("live-one", iso, "in"), "in", iso, iso,
        ))
        con.commit()

    with patch.object(games, "_db", lambda: _connection(path)):
        slate, _ = predictions._sports_slate("nhl")
        response = asyncio.run(predictions.post_sports_pick(
            FakeRequest({"league": "nhl", "matchKey": "nhl:live-one", "side": "A"}),
            "device",
        ))

    assert slate[0]["live"] is True
    assert response.status_code == 409
    assert _body(response)["error"] == "game is locked"
...
def test_non_draw_league_ties_void_both_sides_end_to_end(tmp_path):
    ...
    ended = dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=1)
    iso = ended.isoformat()
    leagues = ("nfl", "ncaaf", "nba", "mlb", "nhl", "atp", "wta")
    with closing(_connection(path)) as con:
    ...
    assert report == {
        "settled": 14, "voided": 14, "pending": 0, "unsettleable": [],
    }
    with closing(_connection(path)) as con:
        rows = con.execute(
