...
def _snapshot_match(league, row):
    ...
    return {
        ...
        "seedA": away.get("seed"),
        "seedB": home.get("seed"),
        "live": game.get("state") == "in",
        "finished": game.get("state") == "post",
...
def _sports_slate(league, now_ms=None):
    """Nearest stored open slate for one league; never calls a publisher."""
    ...
    today = dt.datetime.fromtimestamp(now_ms / 1000, tz=dt.timezone.utc).date().isoformat()
    with closing(_db()) as con:
        rows = con.execute(
            "SELECT game_date, game_id, payload, start_time"
            " FROM scoreboard_snapshots"
            " WHERE league=? AND state='pre' AND game_date>=?"
            " AND start_time>? ORDER BY game_date, start_time, game_id",
            (league, today, now_iso),
        ).fetchall()
        ...
        if not matches:
            ...
            matches = [match for row in rows for match in [_prop_game_match(league, row)] if match and match["startTime"] > now_ms]
    if not matches:
        return [], None
    first_date = min(match["eventDate"] for match in matches)
    slate = [match for match in matches if match["eventDate"] == first_date]
    slate.sort(key=lambda match: (match["startTime"], match["matchKey"]))
    return slate, slate[0]["source"] if slate else None
...
def sports_predict_slate(league: str = Query("mlb")):
    lg = league.lower()
    if lg not in _SPORT_LABELS:
        raise HTTPException(404, f"unsupported prediction league {league!r}")
    selected, source = _sports_slate(lg)
    options = []
    for slug, label in _SPORTS:
        matches, _ = _sports_slate(slug)
        options.append({
            "slug": slug, "label": label, "match_count": len(matches),
            "live_count": 0, "result_count": 0,
            "next_start": matches[0]["startTime"] if matches else None,
        })
    return {
        "schema_version": "sports-predict-v1",
        "selected_title": {"slug": lg, "label": _SPORT_LABELS[lg]},
        "titles": options,
        "matches": selected,
