              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                        ...
                        <button
                          ...
                          onClick={() =>
                            setOpenHold((prev) => {
                              ...
                              if (next.has(card.ticker)) next.delete(card.ticker)
                              else next.add(card.ticker)
                              return next
                            })
                          }
                          aria-expanded={openHold.has(card.ticker)}
                          title={card.hold.why}
                          className={`rounded px-1.5 py-0.5 text-[11px] font-semibold uppercase tracking-wider tabular-nums focus:outline-none focus-visible:ring-1 focus-visible:ring-zinc-400 ${
                            // The one saturated colour marks ABSENCE, never achievement.
                            // A thin sample is the hole here, so it gets the accent; the
                            // present states ramp by brightness instead, dimmer meaning
                            // less floor under the price.
                            card.hold.verdict === 'INSUFFICIENT'
                              ? 'bg-amber-500/15 text-amber-300'
                              : card.hold.verdict === 'PASS'
                                ? 'bg-zinc-800 text-zinc-100'
                                : card.hold.verdict === 'WEAK'
                                  ? 'bg-zinc-800 text-zinc-400'
                                  : 'bg-zinc-800 text-zinc-500'
                          }`}
                        >
                          Hold{' '}
                          {card.hold.verdict === 'INSUFFICIENT' || card.hold.mine.pct == null
                            ? '—'
                            : `${Math.round(card.hold.mine.pct)}%`}
                          <span className="ml-1 font-normal normal-case tracking-normal opacity-60">
                            {card.hold.mine.served}g
                          </span>
                        </button>
                      )}
                      {i === 0 && (
                    ...
                    {/* Price sits WITH the name. It was three blocks down under the
                        model scores, which is fine for reading a card and useless
                        for acting on one while a match is live. The ask is the
                        number the trade is taken at, so it leads. */}
                    <h2 className="mt-1.5 flex flex-wrap items-baseline gap-x-2.5 text-base font-bold leading-snug text-zinc-100">
                ...
                {card.usopen && (
                  <div className="mt-3 rounded-md border border-sky-500/25 bg-sky-500/5 px-3 py-2">
                    ...
                    {card.hold && openHold.has(card.ticker) && (
                      <div className="mt-2 border-t border-sky-500/20 pt-2">
                        ...
                        {/* Sample size rides with every percentage, always. 100% off
                            one service game is not a 100% hold, and the denominator
                            is the only thing that says so. */}
                        {[
                          { who: side, h: card.hold.mine, me: true },
                          { who: opponent || 'opponent', h: card.hold.theirs, me: false },
                        ].map(({ who, h, me }) => (
                          <div
                            key={me ? 'mine' : 'theirs'}
                            className="mt-1 flex items-baseline justify-between gap-3 text-[13px]"
                          >
                            <span className={me ? 'text-zinc-200' : 'text-zinc-500'}>{who}</span>
                            <span className="tabular-nums">
                              <span className={me ? 'font-bold text-zinc-100' : 'text-zinc-400'}>
                                {h.pct == null || h.served === 0 ? '—' : `${Math.round(h.pct)}%`}
                              </span>
                              <span className="ml-1.5 text-zinc-600">
                                held {h.held} of {h.served}
                              </span>
                            </span>
                          </div>
                        ))}
                        ...
                        <p className="mt-2 text-[12px] leading-snug text-zinc-500">
                          Holding so far in <span className="text-zinc-400">this match</span>, not a
                          season rate.{' '}
                          {card.hold.verdict === 'INSUFFICIENT'
